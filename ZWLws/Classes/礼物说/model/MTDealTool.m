...
+ (NSArray *)collectDeals:(int)page
{
    ...
    while (set.next) {
        ZWPresentModel *deal = [NSKeyedUnarchiver unarchiveObjectWithData:[set objectForColumnName:@"deal"]];
...
+ (void)addCollectDeal:(ZWPresentModel *)deal
{
    ...
    [_db executeUpdateWithFormat:@"INSERT INTO t_collect_deal(deal, deal_id) VALUES(%@, %@);", data, deal.ID];
}

+ (void)removeCollectDeal:(ZWPresentModel *)deal
{
    [_db executeUpdateWithFormat:@"DELETE FROM t_collect_deal WHERE deal_id = %@;", deal.ID];
}

+ (BOOL)isCollected:(ZWPresentModel *)deal
{
    FMResultSet *set = [_db executeQueryWithFormat:@"SELECT count(*) AS deal_count FROM t_collect_deal WHERE deal_id = %@;", deal.ID];
    [set next];
//#warning 索引从1开始
